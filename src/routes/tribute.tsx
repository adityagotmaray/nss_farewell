...
// ==========================================================
// BACKEND: MANAGE YOUR MULTI-EVENT TIMELINE HERE
// ==========================================================
const TIMELINE = [
  { 
    year: "2023", 
  ...
  // 💡 Add more years and events by copying the structure above!
];
        ...
        <section className="min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20">
          <Reveal>
            <div className="mb-6 px-4 py-1 border border-accent/30 rounded-full bg-accent/5 backdrop-blur-sm">
                <p className="text-[10px] sm:text-xs uppercase tracking-[0.4em] text-accent font-medium">
                ...
                <div key={yearGroup.year} className={`relative pl-12 sm:pl-0 sm:grid sm:grid-cols-2 sm:gap-20 items-start ${isLeft ? "" : "sm:[&>*:first-child]:order-2"}`}>
                  ...
                  <div className={`${isLeft ? "sm:text-right" : "sm:text-left"} sticky top-40`}>
                    <Reveal>
                        ...
                        <div className={`h-1 w-20 bg-accent/20 my-4 ${isLeft ? "ml-auto" : "mr-auto"}`} />
                    </Reveal>
                  </div>

                  {/* List of Events for that year */}
                  <div className="space-y-20 pt-10">
                    {yearGroup.events.map((event, eventIndex) => (
                        <div key={eventIndex} className="relative">
                            {/* Inner Node for each event */}
                            <div className="absolute left-[-42px] sm:left-[-51px] top-6 h-3 w-3 rounded-full bg-accent/40 border border-accent shadow-[0_0_15px_rgba(212,175,55,0.4)]" />
                            ...
                            <Reveal delay={eventIndex * 150}>
                                <div className="space-y-6">
                                    ...
                                    {event.photo && (
                                        <div className="relative p-2 bg-white/5 border border-white/10 rounded-2xl shadow-2xl rotate-1 hover:rotate-0 transition-transform duration-500 overflow-hidden group">
                                            <img src={event.photo} alt={event.title} className="w-full h-auto rounded-xl grayscale group-hover:grayscale-0 transition-all duration-700" />
                                        </div>
                                    )}
                                </div>
                            </Reveal>
