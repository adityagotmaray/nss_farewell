...
// ============================================================
const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycby69368WqqrW9kSAllBNrpU3evkhjbDIcYNkVYmcfGFX9RwqedgLRoGxR6edy5zW8rO/exec";
      ...
      <div className="relative z-10 pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-16">
            <Reveal>
              ...
              <p className="font-script text-accent text-2xl italic">
                {activeAlbum ? "Step inside the moment." : "Unfolding the chapters of our journey."}
              </p>
            </Reveal>
            {activeAlbumId && (
          ...
          <AnimatePresence mode="wait">
            {loadingVault ? (
              <motion.div key="loader" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-accent/30 animate-pulse mt-20 uppercase tracking-[0.5em] font-display">
                Opening the Vault...
              </motion.div>
            ) : !activeAlbumId ? (
              /* ALBUM LIST */
              <motion.div key="vault" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                {albums.map((album) => (
                  <div key={album.id} onClick={() => setActiveAlbumId(album.id)}
                    className="group cursor-pointer relative aspect-square rounded-[2.5rem] overflow-hidden border border-white/10 bg-white/5 hover:-translate-y-2 transition-all shadow-2xl"
                  >
                          ...
                          <div className="aspect-video w-full relative">
                            {hoveredIndex === i ? (
                              ...
                              <div className="absolute inset-0 w-full h-full">
                                <img src={item.thumbnailUrl} className="w-full h-full object-cover opacity-40 group-hover:opacity-60 transition-opacity" />
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                   <div className="w-16 h-16 rounded-full bg-accent/20 border border-accent/40 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform"><span className="text-accent text-2xl ml-1">▶</span></div>
                                   <p className="text-[9px] text-white/30 uppercase tracking-widest font-black">Hover to Play</p>
                                </div>
                              </div>
                            )}
                          </div>
        ...
        {selectedMedia && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/98 backdrop-blur-2xl flex flex-col items-center justify-center p-4">
            <div className="absolute top-6 right-6 flex gap-4 z-[110]">
               <a href={selectedMedia.downloadUrl} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-white/5 hover:bg-accent hover:text-black flex items-center justify-center transition-all text-white border border-white/10"><Download size={20} /></a>
               <button onClick={() => setSelectedMedia(null)} className="w-12 h-12 rounded-full bg-white/5 hover:bg-red-500 flex items-center justify-center transition-all text-white border border-white/10"><X size={20} /></button>
            </div>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="relative w-full max-w-5xl flex flex-col items-center">
              {selectedMedia.type === "video" ? (<iframe src={selectedMedia.url} className="w-full aspect-video rounded-3xl" allow="autoplay" />) : (<img src={selectedMedia.url} className="max-h-[80vh] w-auto rounded-3xl shadow-2xl border border-white/5" />)}
